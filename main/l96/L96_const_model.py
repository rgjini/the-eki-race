'''
Lorenz 96 model for estimating forcing (1 parameter)
By: Rebecca Gjini 
'''
import numpy as np
...
def lorenz(gamma, x): 
    ...
    out_x = np.zeros((N))
    for kk in range(0, N): 
        out_x[kk] = - x[kk] + (x[(kk + 1)%N] - x[(kk - 2)])*x[(kk - 1)] + gamma[0]
    return out_x
...
def G(gamma, x_0, t, T, ic_cov_sqrtt): 
    '''
    This function is the statistics forward model for the mL96 problem, outputting statistics of 
    the state after observing every other state
    ...
    t : double 
        time step
    T : int # double
        total time to run forward model # time between timesteps
    k : int 
        number of samples 
    spinup : int
        number of samples to take away
    ...
    nx = len(x_0)  
    y = np.zeros(2*nx)
    out = (runge_kutta_v(gamma, x_0 + ic_cov_sqrtt@np.random.normal(0, 1, size = len(x_0)), t, T))[int(4/t):] 
