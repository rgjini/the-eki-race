...
neural network approximation and strong prior
...
def run_iekf(method_type, rmse, ran):
    ...
    for ii in K_vals: 
        ...
        max_runs = 120   #set a maximum number of runs 
        N_t = nx         
        alpha = 1.0 #0.25     #adjustment factor
...
def run_gnco(method_type, rmse, ran):
    ...
    for ii in K_vals: 
        ...
        #GNCO Test 
        try: 
            u_out, f_out, _ = EKA.GN_CO(ml96.G, u, (model, model_input_tensor, x0, t, T, ic_cov_sqrt), 
                                y, R, mu, B, method = method_type, 
                                min_rmse = rmse, tol_x = 1e-4, tol_f = 1e-4, max_iter = max_runs)
        except LinAlgError as a: 
            u_out = np.empty((N_t, K))
